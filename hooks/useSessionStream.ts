...
import { useAppStore, type MessageData, type PartData, type SessionStatus } from "../lib/store";
...
export function useSessionStream() {
  ...
  const fetchInitialData = useCallback(async () => {
    try {
      ...
      const cards = sessions.map((s: { id: string; title?: string; projectID?: string; directory?: string; time?: { created: number } }) => ({
        sessionId: s.id,
        agentName: s.title || `Session ${s.id.slice(0, 8)}`,
      ...
      for (const card of cards) {
        try {
          ...
          if (messages.length > 0) {
            ...
            if (lastMsg.info?.role === "assistant" && !lastMsg.info?.error && lastMsg.info?.time?.completed) {
            ...
            const userMsg = messages.find((m: { info?: { role?: string; agent?: string } }) =>
            ...
            const assistantMsg = messages.findLast((m: { info?: { role?: string; providerID?: string; modelID?: string } }) =>
  ...
  const startSSE = useCallback(() => {
    ...
    try {
      ...
      es.onerror = () => {
        ...
        // Only fall back to polling after a delay (let SSE auto-reconnect first)
        setTimeout(() => {
    ...
    pollRef.current = setInterval(async () => {
      ...
      for (const s of sessions) {
        if (!existingIds.has(s.id)) {
          ...
          if (existing && existing.agentName !== (s.title || existing.agentName)) {
            updateSession(s.id, { agentName: s.title || undefined });
          }
        }
      }
  ...
  }, [addSession, updateSession, removeSession, setConnected, setError]);
  ...
  const handleEvent = useCallback((event: Record<string, unknown>) => {
    ...
    switch (type) {
      case "session.created": {
        const info = props.info as { id: string; title?: string; projectID?: string; directory?: string; time?: { created: number } } | undefined;
        if (info) {
          addSession({
            sessionId: info.id,
            agentName: info.title || `Session ${info.id.slice(0, 8)}`,
      ...
      case "session.updated": {
        ...
        if (info) {
          updateSession(info.id, {
            agentName: info.title,
          });
        }
        break;
      }
      ...
      case "message.updated": {
        const msgInfo = props.info as { id: string; sessionID: string; role: string; time?: { created: number; completed?: number }; error?: unknown; tokens?: { input: number; output: number; reasoning: number }; cost?: number } | undefined;
        if (msgInfo) {
          const existing = useAppStore.getState().sessions.get(msgInfo.sessionID);
      ...
      case "message.part.updated": {
        const part = props.part as { id: string; sessionID: string; messageID: string; type: string; text?: string; delta?: string } | undefined;
        if (part) {
          const existing = useAppStore.getState().sessions.get(part.sessionID);
  ...
  }, [addSession, removeSession, updateSession, addMessage, updateMessage, addPartToMessage, updatePartInMessage, setStatus]);
